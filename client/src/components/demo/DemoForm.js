...
import {KuaidiApi} from "../../api"
import {FormItem, FormTitle, FormInput, Table, TextBox, Row, Col, Button, FormLabel, FormControl} from "../";
const {Form, NumberTextBox, DatePicker, DateTimePicker, DateRange, CheckBox, CheckBoxGroup, Select, Textarea, Radio, AutoComplete} = FormInput;
        ...
        return(
            <Form ref={el => this._form = el} value={value} onChange={this.handleFormChange}>
                ...
                <Row noGutters={true}>
                    ...
                    <Col span={3}>
                        <FormControl>
                            <FormInput.TextBox name="userName" value={value.userName} rules={{required: true, message: "请输入用户名"}}/>
                        </FormControl>
                    </Col>
                    <Col auto={true} offset={1}><FormLabel title="密码：" required={true} /></Col>
                    <Col span={3}>
                        <FormControl>
                            <FormInput.TextBox name="password" value={value.password} type="password" rules={{required: true, message: "请输入密码"}}/>
                        </FormControl>
                    </Col>
                </Row>
                <Row noGutters={true}>
                ...
                <Row label="Table">                    
                    <Col>
                        <FormControl>
                            <FormInput.Table refInput={t => this._customerTable = t}
                                name="customers"
                                minWidth={"800px"}
                                showScroll={false}
                                columns={[
                                    ...
                                    {title: "客户名称", field: "name", editor: <FormInput.TextBox rules={{required: true, message: "必选项"}} popupValidFeedback={true}/>}, 
                                    {title: "所属业务员"}, 
                                    {title: "联系人"}
                                ]}
