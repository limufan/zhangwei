...
namespace Zhangwei.Common
{
    ...
    public class ObjectMapper
    {
        ...
        public ResultType Map<ResultType>(object source)
        {
            if (source != null)
            {
    ...
    public class TObjectMapper<SourceType, ResultType>: ObjectMapper
    {
        Func<SourceType, ResultType> _mapFunc;

        public TObjectMapper(params ObjectMapper[] propertyMappers)
            :base(propertyMappers)
        {
            this._mapFunc = this.Map;
        }

        public TObjectMapper(Func<SourceType, ResultType> mapFunc, params ObjectMapper[] propertyMappers)
            :base(propertyMappers)
        {
        ...
        {
            if (source is SourceType && resultType == typeof(ResultType))
            {
                result = this._mapFunc((SourceType)source);
                return true;
            }
        ...
        public virtual ResultType Map(SourceType source)
        {
            return this.Map<ResultType>(source);
        }
    }
}
